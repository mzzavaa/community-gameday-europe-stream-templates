...
// ── Helpers ──
function useSpringV3(frame: number, fps: number, delay: number) {
  return spring({ frame: frame - delay, fps, config: { damping: 14, stiffness: 120 } });
}

function CountUpV3({ target, frame, startFrame, suffix = "" }: { target: number; frame: number; startFrame: number; suffix?: string }) {
...
// ── Card cover with glassmorphism overlay ──
const CardCoverV3: React.FC<{ name: string; flag: string; accentColor: string }> = ({ name, flag, accentColor }) => {
  const logoUrl = LOGO_MAP[name];
  if (logoUrl) {
    return (
      <div style={{
        width: "100%", flex: 1, minHeight: 0, borderRadius: "16px 16px 0 0",
        overflow: "hidden", position: "relative",
      }}>
        <Img src={logoUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        <div style={{
          position: "absolute", bottom: 0, left: 0, right: 0, height: 50,
          background: "linear-gradient(transparent, rgba(12,8,32,0.8))",
        }} />
      </div>
    );
  }
  return (
    <div style={{
      width: "100%", flex: 1, minHeight: 0, borderRadius: "16px 16px 0 0",
      background: `linear-gradient(135deg, ${accentColor}40, ${GD_DARK})`,
...
const HeroSceneV3: React.FC<{ frame: number; fps: number }> = ({ frame, fps }) => {
  const titleSpring = useSpringV3(frame, fps, 5);
  ...
  const badgeSpring = useSpringV3(frame, fps, 70);
    ...
    <AbsoluteFill style={{ background: GD_DARK }}>
      ...
      <AbsoluteFill style={{ opacity: 0.03 }}>
        <svg width="100%" height="100%">
          <defs>
            <pattern id="hexgridV3" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#c084fc" strokeWidth="0.5" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#hexgridV3)" />
        </svg>
      </AbsoluteFill>
      <div style={{
        ...
        {[
          ...
          <div key={stat.label} style={{ textAlign: "center" }}>
            <div style={{ fontSize: 48, fontWeight: 800, color: "#ffffff", fontFamily: "'Inter', sans-serif" }}>
              {stat.isCountUp ? <CountUpV3 target={stat.value} frame={frame} startFrame={stat.start} suffix={stat.suffix} /> : <>{stat.value}{stat.suffix}</>}
            </div>
...
const GROUPS_PER_PAGE_V3 = 6;
const PAGE_DURATION_V3 = 120;
const TOTAL_PAGES_V3 = Math.ceil(USER_GROUPS.length / GROUPS_PER_PAGE_V3);

const GroupsScrollSceneV3: React.FC<{ frame: number; fps: number }> = ({ frame, fps }) => {
  const TRANSITION_LEAD = 6;
  const adjustedFrame = frame + TRANSITION_LEAD;
  const currentPage = Math.min(Math.floor(adjustedFrame / PAGE_DURATION_V3), TOTAL_PAGES_V3 - 1);
  const pageFrame = adjustedFrame - currentPage * PAGE_DURATION_V3;
  const startIdx = currentPage * GROUPS_PER_PAGE_V3;
  const pageGroups = USER_GROUPS.slice(startIdx, startIdx + GROUPS_PER_PAGE_V3);

  const pageExit = interpolate(pageFrame, [PAGE_DURATION_V3 - 10, PAGE_DURATION_V3], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const progress = (currentPage + 1) / TOTAL_PAGES_V3;

  // Animated glow that shifts position per page
  const glowX = interpolate(currentPage, [0, TOTAL_PAGES_V3 - 1], [20, 80], { extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ background: GD_DARK, fontFamily: "'Inter', sans-serif", color: "#fff" }}>
      ...
      <div style={{
        ...
        transform: "translate(-50%, -50%)",
        background: `radial-gradient(circle, ${GD_PURPLE}18 0%, transparent 70%)`,
        borderRadius: "50%",
      }} />
      ...
      <AbsoluteFill style={{ opacity: 0.02 }}>
        <svg width="100%" height="100%">
          <defs>
            <pattern id="grid2V3" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#c084fc" strokeWidth="0.3" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#grid2V3)" />
        </svg>
      </AbsoluteFill>
      ...
      <div style={{
        position: "absolute", top: 68, left: 36, right: 36, bottom: 40,
        ...
        {pageGroups.map((group, i) => {
          ...
          return (
            ...
            >
              <CardCoverV3 name={group.name} flag={group.flag} accentColor={accentColor} />
              <div style={{ padding: "10px 14px 12px", display: "flex", flexDirection: "column", gap: 3 }}>
...
const ClosingSceneV3: React.FC<{ frame: number; fps: number }> = ({ frame, fps }) => {
  const titleSpring = useSpringV3(frame, fps, 5);
  const subtitleOpacity = interpolate(frame, [20, 40], [0, 1], { extrapolateRight: "clamp" });
  const logoSpring = useSpringV3(frame, fps, 15);
  const websiteSpring = useSpringV3(frame, fps, 55);
  const screenshotSpring = useSpringV3(frame, fps, 65);
  ...
  const totalScrollSteps = SCROLL_SCREENSHOTS_V3.length - 1;
  ...
  return (
    <AbsoluteFill style={{ background: GD_DARK, fontFamily: "'Inter', sans-serif", color: "#fff" }}>
      ...
      <AbsoluteFill style={{ opacity: 0.025 }}>
        <svg width="100%" height="100%">
          <defs>
            <pattern id="grid3V3" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#c084fc" strokeWidth="0.5" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#grid3V3)" />
        </svg>
      </AbsoluteFill>
          ...
          <div style={{
            ...
            <div style={{ width: CLOSING_BROWSER_W, height: CLOSING_VIEWPORT_H, overflow: "hidden", position: "relative" }}>
              <div style={{ display: "flex", flexDirection: "column", transform: `translateY(${scrollY}px)` }}>
                {SCROLL_SCREENSHOTS_V3.map((filename, i) => (
                  <Img
                    key={i}
...
// ── Duration constants ──
const HERO_DURATION_V3 = 150;
const SCROLL_DURATION_V3 = TOTAL_PAGES_V3 * PAGE_DURATION_V3; // 10 × 120 = 1200
const CLOSING_DURATION_V3 = 500;
export const TOTAL_DURATION_V3 = HERO_DURATION_V3 + SCROLL_DURATION_V3 + CLOSING_DURATION_V3; // 1850

// ── Main Composition ──
export const CommunityGamedayEuropeV3: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  return (
    <AbsoluteFill style={{ background: GD_DARK, fontFamily: "'Inter', sans-serif" }}>
      <Sequence from={0} durationInFrames={HERO_DURATION_V3}>
        <HeroSceneV3 frame={frame} fps={fps} />
      </Sequence>
      <Sequence from={HERO_DURATION_V3} durationInFrames={SCROLL_DURATION_V3}>
        <GroupsScrollSceneV3 frame={frame - HERO_DURATION_V3} fps={fps} />
      </Sequence>
      <Sequence from={HERO_DURATION_V3 + SCROLL_DURATION_V3} durationInFrames={CLOSING_DURATION_V3}>
        <ClosingSceneV3 frame={frame - HERO_DURATION_V3 - SCROLL_DURATION_V3} fps={fps} />
      </Sequence>
    </AbsoluteFill>
  );
};
