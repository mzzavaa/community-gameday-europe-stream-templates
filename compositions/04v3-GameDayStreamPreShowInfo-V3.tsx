/**
 * 04v3 – GameDayPreShowInfoV3 (Muted)
 *
 * 30-minute cinematic pre-show loop — V3 improvements over V2:
...
const SlideMeetLinda: React.FC = () => {
  ...
  const points = [
    { entry: p1, text: "AWS Community Hero — the highest recognition tier for community advocates" },
    { entry: p2, text: "Leader of AWS User Group Vienna & AWS Women's User Group Vienna" },
    { entry: p3, text: "Co-organizer and host of today's live stream across 57 European cities" },
    { entry: p4, text: "Member of Förderverein AWS Community DACH e.V." },
  ];
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 48, width: "100%", maxWidth: 1060 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ opacity: textE, transform: `translateX(${interpolate(textE, [0, 1], [22, 0])}px)`, marginBottom: 20 }}>
            ...
            <div style={{ fontSize: TYPOGRAPHY.h6, color: GD_ACCENT, marginTop: 8, fontWeight: 600 }}>Stream Host · AWS Community Hero · Co-Organizer</div>
          </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 48, width: "100%", maxWidth: 1060 }}>
        ...
        <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
          <div style={{ opacity: heroE, transform: `scale(${interpolate(heroE, [0, 1], [0.85, 1])})` }}>
            <div style={{ width: 180, height: 180, borderRadius: "50%", overflow: "hidden", border: `3px solid ${GD_GOLD}66`, boxShadow: `0 0 48px ${GD_GOLD}33` }}>
              <Img src={staticFile(linda.face)} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
          </div>
          <div style={{ opacity: heroE, display: "flex", alignItems: "center", gap: 6, background: `${GD_GOLD}22`, border: `1px solid ${GD_GOLD}55`, borderRadius: 10, padding: "6px 14px" }}>
            <ShieldIcon size={14} color={GD_GOLD} />
            <span style={{ fontSize: 12, fontWeight: 700, color: GD_GOLD, letterSpacing: 1.5, textTransform: "uppercase" }}>AWS Hero</span>
          </div>
          <div style={{ opacity: textE, fontSize: 16, fontWeight: 800, color: "white", textAlign: "center" }}>Linda Mohamed</div>
          <div style={{ opacity: textE, fontSize: 12, color: GD_ACCENT, textAlign: "center" }}>AWS Community Hero · Vienna</div>
        </div>

        {/* Explanation */}
        <div style={{ flex: 1 }}>
          <div style={{ opacity: textE, transform: `translateX(${interpolate(textE, [0, 1], [22, 0])}px)`, marginBottom: 20 }}>
            <div style={{ fontSize: TYPOGRAPHY.h4, fontWeight: 900, color: "white", lineHeight: 1 }}>The highest community recognition</div>
  ...
  return (
    <AbsoluteFill style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ opacity: flagE, transform: `scale(${interpolate(flagE, [0, 1], [0.7, 1]) * pulse})`, marginBottom: 20 }}>
        {logo ? (
          <div style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${GD_VIOLET}33`, borderRadius: 20, padding: "20px 36px", display: "flex", alignItems: "center", justifyContent: "center", minWidth: 320, minHeight: 140 }}>
...
// ROOT EXPORT
// ═══════════════════════════════════════════════════════════════════════════════
export const GameDayPreShowInfoV3: React.FC = () => {
  const frame = useCurrentFrame();
